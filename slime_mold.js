var scriptActiveSlime = true;
var decay_slime = 0.1;
var sensor_slime = 4;
var abs_speed_slime = 3;
var angle_part_slime = 0.25;

(function(w) 
{
	///////////////////////////
	var numParticlesSlime = 5000;
	
	var diff = 0.5;

	var slime_part_shape = 0;
	
	var angle = angle_part_slime;
	...
	_Particles.prototype = 
	{
		setSpeed:function(speed)
		{
			...
			this._speedL = V(Math.cos(this.alfa+this.lambda), Math.sin(this.alfa+this.lambda));
			this._speedR = V(Math.cos(this.alfa-this.lambda), Math.sin(this.alfa-this.lambda));
		...
		update: function(dir) 
		{
			// Aggiorno direzione
			if (dir == 2)
				this.alfa -= this.lambda;			
			else if(dir==1)
				this.alfa += this.lambda;
			if(!dir == 0)
				this.setSpeed(abs_speed_slime);
			if(this.pos.x + this._speed.x < 0)
				this.pos.x+=ww;
			else if(this.pos.x + this._speed.x > ww)
				this.pos.x-=ww;
			else if(this.pos.y + this._speed.y < 0)
				this.pos.y+=wh;
			else if(this.pos.y + this._speed.y > wh)
				this.pos.y-=wh;
	...
	function onTouchMove(e)
	{
		...
		for (var i = -mouseWidth; i < mouseWidth; i++)
		{
			...
			{
				diffuseParticle(Math.floor(e.touches[0].clientX+i), Math.floor(e.touches[0].clientY+j), 10000000);
			}
		}
	}
	...
	function onMouseDown(e)
	{
		...
		for (var i = -mouseWidth; i < mouseWidth; i++)
		{
			for (var j = -mouseWidth; j < mouseWidth; j++)
			{
				diffuseParticle(e.clientX+i-offsetX, e.clientY+j-offsetY, 10000000);
			}
		}
		mousedown = true;
	}
	...
	{
		var xs = Math.floor((p.pos.x + sensor_slime*p._speed.x)/gridStep)%gridX;
		if (xs<0) xs+= gridX;
		var ys = Math.floor((p.pos.y + sensor_slime*p._speed.y)/gridStep)%gridY;
		if (ys<0) ys+= gridY;
		
		var xl = Math.floor((p.pos.x + sensor_slime*p._speedL.x)/gridStep)%gridX;
		if (xl<0) xl+= gridX;
		var yl = Math.floor((p.pos.y + sensor_slime*p._speedL.y)/gridStep)%gridY;
		if (yl<0) yl+= gridY;
		
		var xr = Math.floor((p.pos.x + sensor_slime*p._speedR.x)/gridStep)%gridX;
		if (xr<0) xr+= gridX;
		var yr = Math.floor((p.pos.y + sensor_slime*p._speedR.y)/gridStep)%gridY;
		if (yr<0) yr+= gridY;
		...
		if (left == right)
		{
			if (!left >= streight) return 0; //dritto
			else return Math.floor(2*Math.random())+1;
		}
		else if (left > right)
		{
			if (left >= streight) return 1; //sx
			else return 0;
		}
		else
		{
			if (right >= streight) return 2; //dx
			else return 0;	
		}
	}
	...
	function loopSlime() 
	{
		...
		{
			if (angle_part_slime!=angle)
			{
				angle = angle_part_slime;
				refreshSlime();
			}
			var i, len, g, p;
			contextSlime.save();
			...
			bufferCtx.lineWidth = (slime_part_shape == 0) ? PARTICLE_RADIUS * (2 + rnd) : 3*PARTICLE_RADIUS * (2 + rnd);
			bufferCtx.beginPath();
			for (i = 0; i < len; i++) 
			{
				p = _particles[i];
				...
				// aggiorno la griglia
				diffuseParticle(Math.floor((p.pos.x)/gridStep)%gridX,Math.floor((p.pos.y)/gridStep)%gridY, 1 /*+(p.pheromone*transport)*/);
			}
			bufferCtx.stroke();
			bufferCtx.restore();
